    ...
    def get_image_placeholder_mask(
        ...
        if input_ids is None:
            if inputs_embeds is None:
                raise ValueError(
                    "Either `input_ids` or `inputs_embeds` has to be provided."
                )
...
class T5Gemma2Decoder(T5Gemma2PreTrainedModel):
    ...
    def __init__(self, config: T5Gemma2DecoderConfig, eoi_token_index: int = 256_000):
        ...
        self.layers = nn.ModuleList(
            [
                T5Gemma2DecoderLayer(config, layer_idx)
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if not self.training and use_cache and past_key_values is None:
            past_key_values = EncoderDecoderCache(
                DynamicCache(config=self.config), DynamicCache()
            )
        ...
        if not isinstance(self_attn_mask_mapping := attention_mask, dict):
            ...
            mask_kwargs["and_mask_function"] = lambda *args: torch.tensor(
...
class T5Gemma2ForConditionalGeneration(T5Gemma2PreTrainedModel, GenerationMixin):
    ...
    def _prepare_cache_for_generation(
        ...
        if past_key_values is not None:
            ...
            if len(past_key_values.is_updated) > 0 and past_key_values.is_updated.get(
                0
            ):
                return
            ...
            if cross_attn_cls == StaticCache:
                cross_attn_cache_kwargs["max_cache_len"] = model_kwargs[
                    "encoder_outputs"
                ][0].shape[1]
            # Update cross-attention cache only (switch from sliding_window to full).
            past_key_values.cross_attention_cache = cross_attn_cls(
                **cross_attn_cache_kwargs
            )
        else:
        ...
        if hasattr(self, "_cache") and self._cache is not None:
            if not isinstance(self._cache, EncoderDecoderCache):
                raise ValueError(
                    "The internal cache must be of type `EncoderDecoderCache` for T5Gemma2 model."
                )
...
class T5Gemma2ForSequenceClassification(T5Gemma2PreTrainedModel):
    def __init__(self, config: T5Gemma2Config):
        ...
        self.score = T5Gemma2ClassificationHead(
            self.hidden_size, self.num_labels, classifier_dropout
        )
        self.post_init()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        non_pad_mask = (decoder_input_ids != self.config.pad_token_id).to(
            logits.device, torch.int32
        )
        token_indices = torch.arange(
            decoder_input_ids.shape[-1], device=logits.device, dtype=torch.int32
        )
