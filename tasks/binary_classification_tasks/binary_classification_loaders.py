...
def load_binary_classification_label_based(task, rank=0):
    ...
    if not hasattr(task, "label_texts"):
        raise ValueError(f"Task {task.__class__.__name__} must define 'label_texts' dict for label-based loading")
    ...
    if rank == 0:
        print(f"Loaded {len(query_texts)} query-positive pairs for binary classification")
...
def load_binary_classification_hard_negatives(task, rank=0):
    ...
    if rank == 0:
        print(f"Loaded {len(query_texts)} query-positive pairs for binary classification with hard negative mining")
