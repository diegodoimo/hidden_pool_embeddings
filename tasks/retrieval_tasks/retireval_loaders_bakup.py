...
def from_one_hf_dataset(task, max_num_queries=10**6) -> RetrievalRawData:
    """
    ...
    if rank == 0:
        ...
        print(
            f"Positives referenced by filtered pairs: {return_formatted(len(full_positive_ids))}"
        )
        print(
            f"Total number of queries with repetitions {return_formatted(len(full_query_ids))} (under {max_num_queries//10**6}M limit)"
        )
        print(
