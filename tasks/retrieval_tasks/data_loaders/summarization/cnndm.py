...
def load_cnndm_retrieval(task, max_num_queries=10**6, rank=None) -> RetrievalRawData:
    """Load CNN/DailyMail summarization dataset for retrieval using vectorized operations.
    ...
    df = dataset.select_columns(["highlights", "article"]).to_pandas()
    n_pairs = len(df)
    ...
    unique_query_mask = ~df["highlights"].duplicated(keep="first")
    unique_query_idx = unique_query_mask[unique_query_mask].index.values
    unique_query_texts = df.loc[unique_query_mask, "highlights"].tolist()
    unique_query_ids = [f"query_{i}" for i in unique_query_idx]
    
    # Create query text to ID mapping
    query_text_to_id = pd.Series(unique_query_ids, index=df.loc[unique_query_mask, "highlights"].values)
    
    # Map all queries to their unique IDs
    query_ids = df["highlights"].map(query_text_to_id).tolist()
    
    # Get unique positives
    unique_positive_mask = ~df["article"].duplicated(keep="first")
    unique_positive_idx = unique_positive_mask[unique_positive_mask].index.values
    unique_positive_texts = df.loc[unique_positive_mask, "article"].tolist()
    unique_positive_ids = [f"doc_{i}" for i in unique_positive_idx]
    
    # Create document text to ID mapping
    doc_text_to_id = pd.Series(unique_positive_ids, index=df.loc[unique_positive_mask, "article"].values)
    
    # Map all positives to their unique IDs
    positive_ids = df["article"].map(doc_text_to_id).tolist()
