...
def from_one_hf_dataset(task, max_num_queries=10**6, rank=None) -> RetrievalRawData:
    """
...
def from_multiple_hf_datasets(task, max_num_queries=10**6, rank=None) -> RetrievalRawData:
    """
    ...
    if max_num_queries is not None and len(unique_query_idx) > max_num_queries:
        ...
        if remaining_corpus_ids:
            ...
            document_texts.extend([corpus_dict[did]["text"] for did in remaining_corpus_ids])
            if has_title:
                document_titles.extend([corpus_dict[did]["title"] for did in remaining_corpus_ids])
    ...
    assert corpus_keys == doc_keys, f"corpus_dict keys mismatch with document_ids. Missing in corpus: {doc_keys - corpus_keys}, Missing in docs: {corpus_keys - doc_keys}"
